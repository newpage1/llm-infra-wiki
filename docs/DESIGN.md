...
**分章**：`section`（写在 front-matter 里）决定列表页的**章节**，顺序固定为
`lmcache` / `mooncake` / `sglang` / `vllm` / `新模型`，没有内容的章节不渲染；
不写 `section` 的落进最后的「未归类」段。

章节表（名字 + 那句说明 + 顺序）**只写一份**，在 `tools/build_flows.js` 的 `SECTIONS`：
生成器按它校验 front-matter（写错直接报错并列出合法值），再把它写进
`flows/manifest.json`，列表页照单渲染分段——页面里不抄第二份。

另有一层**可选的**子目录分组（如 `flows/vllm-ascend/`），只影响文件摆放，
显示为卡片上的一枚标签。曾经拿它当分类轴（「技术方向」），划分与维护成本都不划算，
已去掉——分类现在只有 `section` 这一个轴。
