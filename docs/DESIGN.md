...
曾经有一个独立的 `#/flows` 联动分析页，放三条跨组件链路（KV Save/Load 全链路、
KV 回落的三条通路、一个 KV 对象的四次变形），配 `data/flows.js` 与 `.seam-table`
这类专用版式。**已经拆掉了**，理由和替代方案如下。

**为什么拆**：那三条链路的体裁各不相同（调用路径 / 方案对比 / 数据变换），却挤在
同一套「列表页 + 详情页」的模子里，版式要为它们仨同时让路；而它们的内容量又不足以
撑起一个和六个深度分析页并列的一级栏目。加上维护成本——每改一次组件页的接口，
联动页的接缝表就要跟着核对一遍。

**现在怎么放**：拆成 `notes/` 下的调研笔记。笔记是自由形式的，不要求八节骨架，
所以「调用路径」可以就是一张流程图加几段，「方案对比」可以是几张表，互不迁就。
代价是它们不再有统一入口——`#/n` 列表页只按日期倒序列出，读者靠标题和标签找。

**谁提交、怎么进去**：笔记由同事直接提 `.md` 到 `notes/`，站点是纯静态的、
没有后端，Pages 又不会列目录，所以列表页读的 `notes/manifest.json` 必须有人生成——
由 [`.github/workflows/notes.yml`](../.github/workflows/notes.yml) 在 `notes/**`
的 push 上扫目录重建并提交（方案 B）。另外两条路（运行时调 GitHub Contents API
列目录、改成 Pages workflow 部署源）分别受限于 IP 限流和需要手工改 Settings，没选。

**深度分析与跨组件分析的分工**（这条没变）：

**深度分析**跟着一个仓库走（单组件，钉一个版本）。
跨组件的分析跟着一次操作走（跨仓库，钉每个参与方的版本）。
